PO<-function(Obs,Pred)
{
  ...
  colnames(data) <- c("Predictions","Legend","Times")
  
  g2 <- ggplot(data, aes(Times, Predictions,  colour=Legend))+ylab("Fire spots")+xlab("Date")+ggtitle("")
  ...
  g2 <- g2 + annotate("text", x = data$Times[dim(dataset)[1]*0.2], y=max(Obs), label = "Training set", size=6)
  g2 <- g2 + annotate("text", x = data$Times[dim(dataset)[1]*0.8], y=max(Obs), label = "Test set", size=6)
  g2 <- g2 + geom_vline(aes(xintercept=as.numeric(Times[dim(dataset)[1]*0.7])),size = 1, color ="black")
