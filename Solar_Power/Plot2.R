Plot2<-function(date,Obs,Pred1,Pred2,Pred3) {
  ...
  g2 <- g2 + annotate("text", x = data$Date[dim(Obs)[1]*0.2], y=max(Obs)*0.1, label = "Training set", size=6)
  g2 <- g2 + annotate("text", x = data$Date[dim(Obs)[1]*0.8], y=max(Obs)*0.1, label = "Testing set", size=6)
  g2 <- g2 + geom_vline(aes(xintercept=as.numeric(Date[dim(Obs)[1]*0.7])), size = 1, color ="black")
